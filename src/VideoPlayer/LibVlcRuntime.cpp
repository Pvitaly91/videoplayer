...
#define RESOLVE(member, symbol)                                      \
    if (!Resolve(vlcModule_, #symbol, api_.member, error)) {          \
        ...
        RESOLVE(mediaAddOption, libvlc_media_add_option)
        ...
        RESOLVE(mediaPlayerGetState, libvlc_media_player_get_state)
        RESOLVE(videoGetTrackCount, libvlc_video_get_track_count)
